...
int main(int argc, char ** argv)
{
	...
	if (optValue and filePath){
		...
		uchar* keys;
		pthread_t tid;
	    pthread_create(&tid, NULL, vectorCipher, NULL);
...
void *vectorCipher(void * vargp){
	void* virtual_base;
	void* mem_addr;
	fd=open("/dev/mem",(O_RDWR|O_SYNC));
	virtual_base=mmap(NULL,REG_SPAN,(PROT_READ|PROT_WRITE),MAP_SHARED,fd,REG_BASE);
	mem_addr=virtual_base+VECTORIAL_MEM_0_BASE;
	uchar * char_iter = (uchar*)(mem_addr+1);
	for(int i =0; i < chunk_size; i++){
		*char_iter=*(uchar *)(vargp+i);
	}
	*(uchar*)mem_addr='1';
	while(*(uchar*)mem_addr=='1'){
		sleep(1);
	}
	char_iter = (uchar*)(mem_addr+1);
	for(int i =0; i < chunk_size; i++){
		*(uchar*)(vargp+i)=*(char_iter+i+1);
	}
	return;
}
